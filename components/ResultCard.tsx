      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white/60 backdrop-blur-md rounded-3xl p-8 border border-white shadow-lg flex flex-col justify-between h-full">
          <div>
            ...
            <div className="py-4 flex items-center justify-center gap-8">
              <div className="flex flex-col items-center w-24">
                ...
                <span className="text-5xl font-bold text-stone-800 tracking-tight">
                  {Math.round(result.weather.minTemp)}°
                </span>
              </div>
              ...
              <div className="flex flex-col items-center w-24">
                ...
                <span className="text-5xl font-bold text-stone-800 tracking-tight">
                  {Math.round(result.weather.maxTemp)}°
                </span>
              </div>
            </div>
            ...
            {/* Token Usage Info */}
            {result.weather.tokenUsage && (
              <div className="mt-4 pt-4 border-t border-stone-100">
                <p className="text-stone-400 text-[10px] uppercase mb-2">Token Usage</p>
                <div className="flex gap-4 text-[10px] text-stone-500">
                  <span>Total: {result.weather.tokenUsage.totalTokens}</span>
                  {result.weather.tokenUsage.promptTokens && <span>Prompt: {result.weather.tokenUsage.promptTokens}</span>}
                  {result.weather.tokenUsage.candidatesTokens && <span>Response: {result.weather.tokenUsage.candidatesTokens}</span>}
                </div>
              </div>
            )}
